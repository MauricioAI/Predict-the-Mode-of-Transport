    ...
    if os.path.isfile(dir_src + "/" + file):
        with open(dir_src + "/" + file) as current_file:
            if file.endswith(".zip"):
                ...
                acc_file = glob.glob("./**/SenseMyCity-" + session + "-gyroscope.csv", recursive=True)
                if len(acc_file) == 0:
                        ...
                        with open(acc_file[0]) as current_csv_file:
                            for line in current_csv_file:
                                ...
                                if len(values) > 4 and values[0].isnumeric():
                                    recolhas_gyroscope.append((values[1], values[2], values[3], values[4]))
                        if len(recolhas_gyroscope) > 0:
                            gyr_out = []
                            for i in range(len(recolhas_gyroscope)):
                                dec_gyrx = calculateAverage(recolhas_gyroscope[i][1].replace('\\', '0').replace('"', ''))
                                dec_gyry = calculateAverage(recolhas_gyroscope[i][2].replace('\\', '0').replace('"', ''))
                                dec_gyrz = calculateAverage(recolhas_gyroscope[i][3].replace('\\', '0').replace('"', ''))
                                gyr_out.append((recolhas_gyroscope[i][0],sqrt(dec_gyrx+dec_gyry+dec_gyrz)))

                        if (len(gyr_out) > 0):
                            #  insert multiple points into the recolhas table  """
                            sql1 = "INSERT INTO recolhas_gyroscope(id_recolha, actividade, id_user, instante, gyroscope) VALUES (" + session + ",'" + activity + "'," + user_id + ",%s,%s)"
                            conn = None
                            try:
